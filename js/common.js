...
const BirdApp = {
  ...
  /** B站视频封面缓存 (bvid → cover URL) */
  _bilibiliCache: {},

  /** 根据 bvid 自动获取 B 站视频封面，带缓存 */
  async fetchBilibiliCover(bvid) {
    if (!bvid) return null;
    if (this._bilibiliCache[bvid]) return this._bilibiliCache[bvid];
    try {
      ...
      const cover = json?.data?.pic || null;
      if (cover) this._bilibiliCache[bvid] = cover;
      return cover;
    } catch {
      return null;
    }
